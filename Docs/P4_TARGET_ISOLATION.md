...
本轮建立了“从马群中选定一匹并把它切离群体”的最小玩法闭环。玩家朝向一匹野马按 `Q`，系统从前方 60 米、视线夹角范围内选择最合适的成员；再次对同一目标按 `Q` 可取消。

被选中的马继续使用自己的感知、状态机、障碍探测、动态避让和 CMC 运动，但暂时不接受群体中心与群体平均方向的牵引，仍保留成员间分离力。这使玩家可以用坐骑从侧后方施压，把目标逐步切出，同时避免目标穿过其他马匹。

## 完成条件与反馈

- 默认要求目标距其余马群中心至少 18 米。
- 在距离外持续 2 秒完成隔离；回到距离内时进度以双倍速度衰减。
...
3. 观察 HUD 的 separation；超过 18.0 m 后保持 2 秒，直到显示 `ISOLATED`。
