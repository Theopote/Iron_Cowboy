...
1. 用 Q 选择一匹野马，并把它切到距其余马群中心至少 18 米、保持 2 秒，直到 HUD 显示 `ISOLATED`。
2. 按住鼠标右键进入 Aiming。未选择目标或目标尚未隔离时，右上角会说明原因。
3. 保持准星对准目标，按鼠标左键投掷。套索速度为 3200 cm/s，最大射程 26 米，命中半径 80 cm。
4. 命中后进入 Attached，目标进入 `Horse.State.Lassoed` 并通过原有 HorseMovement 紧急制动。
5. Attached 时按鼠标左键释放。脱靶、击中障碍、目标消失或绳长超过限制都会进入 0.75 秒 Recovering，随后重新可用。
...
`Steppe.P5.LassoThrowAttachAndRecovery` 覆盖未隔离拒绝、瞄准、投掷扫掠、目标命中、Lassoed 行为、Gameplay Tag、主动释放、脱靶和自动恢复。
